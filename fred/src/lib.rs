...
use std::{ffi::*, os::windows::prelude::MetadataExt};
...
use crate::offsets::*;
use hooking::{inline_hook, replace_hook};
use winapi::{
    shared::minwindef::{DWORD, HINSTANCE},
    um::{minwinbase::CRITICAL_SECTION, winnt::DLL_PROCESS_ATTACH},
};
...
extern "cdecl" fn free_dummy_file_info(ctx: hooking::InlineCtx) {
    unsafe {
...
static mut MOUNT_PAK_FILE_FUNC_PTR: *mut extern "thiscall" fn(*mut PakSystem, *const i8) = unsafe {
    std::mem::transmute::<_, *mut extern "thiscall" fn(this: *mut PakSystem, path: *const i8)>(
        0x00687a60 as *const (),
    )
};
...
extern "stdcall" fn DllMain(_h_inst_dll: HINSTANCE, fdw_reason: DWORD, _lpv_reserved: *mut c_void) {
    if fdw_reason == DLL_PROCESS_ATTACH {
        logging::init();
        unsafe {
            replace_hook(&mut LOAD_FILE_BY_PATH_FUNC_PTR, load_file_by_path as *mut _);
            replace_hook(&mut MOUNT_PAK_FILE_FUNC_PTR, mount_pak_file as *mut _);
